...
mqtt.on('connect', () => {
  ...
  MongoClient.connect('mongodb://db', (err, db) => {
    ...
    io.on('connection', socket => {
      ...
      socket.on('joinDextersLab', data => {
        socket.join(`dextersLab/${data.username}`)
        db.collection('devices')
          .find({ name: 'Dexter\'s Lab' }, (err, device) => {
            if (err) console.log(err)
            db.collection('statuses')
              .find({ deviceID: device.deviceID })
              .toArray((err, events) => {
                if (err) console.log(err)
                io.sockets.in(`dextersLab/${data.username}`)
                  .emit('initDextersLab', events)
              })
          })
      })
